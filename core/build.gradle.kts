        ...
        externalNativeBuild {
            cmake {
                ...
                arguments("-DRIRU_MODULE_API_VERSION=$moduleMaxRiruApiVersion",
                        "-DRIRU_MODULE_VERSION=${rootProject.extra["versionCode"]}",
                        "-DRIRU_MODULE_VERSION_NAME:STRING=\"${rootProject.extra["versionName"]}\"")
            }
        }

        buildConfigField("int", "API_CODE", "$apiCode")
        buildConfigField("String", "VERSION_NAME", "\"${rootProject.extra["versionName"]}\"")
        buildConfigField("Integer", "VERSION_CODE", rootProject.extra["versionCode"].toString())
    }
...
afterEvaluate {
    ...
    android.applicationVariants.forEach { variant ->
        ...
        val zipFileName = "${moduleName}-${rootProject.extra["versionName"]}-${rootProject.extra["versionCode"]}-${variantLowered}.zip"
            ...
            doFirst {
                copy {
                    ...
                    expand("moduleId" to moduleId,
                            "versionName" to "$rootProject.ext.versionName",
                            "versionCode" to "$rootProject.ext.versionCode",
